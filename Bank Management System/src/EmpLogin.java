...
public class EmpLogin {
	...
	private void initialize() {
		...
		Image img=(new ImageIcon(EmpLogin.class.getResource("/icon/Bank_icon.png"))).getImage();
		...
		JButton btnSignUp_1_1 = new JButton("ADMIN");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					String q="select * from employee where euname = '"+ausername+"' and password = '"+apass+"';";
					try {
						ResultSet rs=c1.s.executeQuery(q);
						
						if(rs.next()){
							aid= Integer.parseInt(rs.getString("eid"));
							new Customer.Home(aid,ausername);
